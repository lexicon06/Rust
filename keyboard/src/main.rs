...
fn main() {
    ...
    io::stdin().read_line(&mut user_name).expect("Something went wrong, can't catch your name"); // we use ref ~ puntero, w "mut" we tell is able to write as well
    let user_name = user_name.trim();// delete odd characters w shadowing
    println!("Enter your age:");
    io::stdin().read_line(&mut user_age).expect("Something went wrong, can't catch your age");
